...
class FoofindApi
{
    ...
    public function getSearch($q, $lang, $src, $type, $size, $year, $brate, $results, $apitoken)
    {
        ...
        if ($SphinxPaginator !== null)
        {
            ...
            foreach ( $paginatorArray as $i => $value)
            {
                ...
                $paginatorArray2[$i]['filename'] = $paginatorArray[$i]['filename'];
                $paginatorArray2[$i]['dlink'] =urldecode( '<![CDATA['.'http://foofind.com/'.$lang.'/download/'.$paginatorArray[$i]['dlink'] .']]>' );
