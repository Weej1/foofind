...
var lastrequest = null, tamingtime=0;var textsearch,tamingresults,docLang;var lastsearch="";function configTaming(a){docLang=a;textsearch=$("#q");$("#q-element").prepend("<div id='tamingresults'></div>");tamingresults=$("#tamingresults");textsearch.blur(delay(hideAutocomplete,200));textsearch.focus(delay(showAutocomplete,200));textsearch.keydown(keypress);textsearch[0].focus()}function taming(){lastsearch=textsearch.val();if(lastsearch.length<2){lastsearch="";hideAutocomplete();return}type=$("#type").val();if(type.length>0)type="&t="+type;if(lastrequest!=null) lastrequest.abort(); lastrequest = $.ajax({url:"/"+docLang+"/taming/autocomplete?q="+escape(lastsearch)+type,dataType:"json",context:lastsearch,error:function(){tamingresults.html("");hideAutocomplete()},success:function(d){lastrequest=null;if(this!=lastsearch)return;if(lastsearch.length<2||d==null||!d){tamingresults.html("");hideAutocomplete();return}res="";t=lastsearch;data=d;for(l in data){if(data[l][0]<0)break;var i=0;for(;i<t.length;i++)if(i>t.length||data[l][2][i]!=t[i])break;res+="<li>"+data[l][2].substr(0,i)+"<b>"+data[l][2].substr(i)+"</b></li>"}if(res.length>0){tamingresults.html("<ul>"+res+"</ul>");$("li",tamingresults).click(selectAutocomplete);$("li, b",tamingresults).mousemove(function(e){$("li.active",tamingresults).removeClass("active");if(e.target.nodeName.toLowerCase()=="li")it=e.target;else it=e.target.parentNode;activate($(it),false)});tamingresults.css("display","block")}else{tamingresults.html("");hideAutocomplete()}}})}function selectAutocomplete(e){if(e.target.nodeName.toLowerCase()=="li")it=e.target;else it=e.target.parentNode;textsearch.val($(it).text());hideAutocomplete();$("form").submit()}function hideAutocomplete(){if(tamingtime!=0)clearTimeout(tamingtime);tamingresults.css("display","none")}function showAutocomplete(){if(lastsearch==""){lastsearch="";tamingresults.html("")}if(tamingresults.html().length==0)tamingresults.css("display","none");else tamingresults.css("display","block")}function delay(f,a){return function(){if(tamingtime!=0) {clearTimeout(tamingtime);}tamingtime=setTimeout(f,a)}}function activate(a,b){if(b)textsearch.val(a.text());a.addClass("active")}function keypress(e){switch(e.which){case 27:e.preventDefault();textsearch.val(lastsearch);hideAutocomplete();break;case 38:e.preventDefault();showAutocomplete();active=$("li.active",tamingresults);last=$("li:last",tamingresults);if(active.length==0)activate(last,true);else{active.removeClass("active");if(active.prev().length>0)activate(active.prev(),true);else activate(last,true)}break;case 40:e.preventDefault();showAutocomplete();active=$("li.active",tamingresults);first=$("li:first",tamingresults);if(active.length==0)activate(first,true);else{active.removeClass("active");if(active.next().length>0)activate(active.next(),true);else activate(first,true)}break;default:delay(taming,150)();break}}
var jimage, jimagecount, thumbani = 0;function animateImage() { src = jimage.attr("src"); idx = parseInt(src.substr(src.length-1)); idx = (idx+1)%jimagecount; src = src.substr(0,src.length-1) + idx.toString(); jimage.attr("src", src); }

$.template = function(h, d){for(k in d){h=h.replace(new RegExp("%"+k+"%",'g'),d[k])}return h}